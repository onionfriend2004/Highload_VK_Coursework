...
| **Redis**  (аналог aerospike можно туда)                                | Сессии и кэширование данных                                                        | In-memory модель с поддержкой TTL обеспечивает быстродействие при авторизации и валидации сессий.                                                  |
...
| **API Gateway**                           | **Реплицирование**                       | Точка входа в систему, поэтому критично обеспечить масштабирование и дублирование. Разделение потоков web/mobile клиентов и равномерное распределение нагрузки предотвращает перегрузку. |
...
| **Aerospike**                             | **Репликация и cold backup**             | Встроенная репликация (11 реплик) обеспечивает высокую доступность при больших объёмах сообщений и участников. Снимки (snapshots) хранятся в S3.                                         |
| **Redis**                                 | **Репликация и Sentinel**                | Master–replica + автоматический failover через Sentinel. Для хранения сессий допускается временная потеря данных, но система быстро восстанавливается.                                   |
| **ClickHouse**                            | **Реплицирование + Backup**              | Использование `ReplicatedMergeTree` и ClickKeeper. Регулярные резервные копии партиций в S3 обеспечивают быструю реконструкцию.                                                          |
| **Kafka**                                 | **Репликация брокеров**                  | Параметр `replication.factor=3` гарантирует сохранность событий даже при падении узлов.                                                                                                  |
| **S3 / Object Storage**                   | **Versioning + Lifecycle rules**         | Включение версионирования и автоматическое хранение копий обеспечивает надёжность пользовательских записей.                                                                              |
| **CDN / Media-сервера (SFU)**             | **Резервирование и геораспределённость** | Несколько точек присутствия для обеспечения низкой задержки и отказоустойчивости. При падении SFU пользователи автоматически переподключаются к ближайшему узлу.                         |
| **Monitoring (VictoriaMetrics, Grafana)** | **Резервирование и алертинг**            | Метрики и логи реплицируются, что позволяет сохранять наблюдаемость даже при частичном отказе компонентов.                                                                               |
...
| ------------------------ | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **Retry**                | Повтор запросов при временных сбоях          | При недоступности SFU/CDN клиент повторно подключается к резервному серверу. Число попыток ограничено для предотвращения перегрузки. |
...
| ---------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------- |
| **Дата-центры (DC)**   | **Геораспределённое резервирование**        | Размещение узлов в разных регионах для защиты от аварий и катастроф.                  |
