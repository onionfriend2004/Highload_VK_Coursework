...
#### Ключевые компоненты
- **Ingress Gateway** — публичная точка входа, принимает медиапотоки от клиентов, маршрутизирует к внутренним сервисам.  
...
1. **Client1 → Signaling Service:** запрос на звонок к Client2.  
2. **Signaling Service** проверяет через **User Service**, можно ли инициировать звонок.  
...
## 2. Обмен сообщениями (чат во время звонка)

### Общая идея
Во время звонка все участники подключены к системе через **WebSocket-обработчики (WSH)** — лёгкие серверы, поддерживающие постоянное соединение с активными пользователями.  

### WebSocket-обработчики и Participants Service
Каждый WSH может обслуживать десятки тысяч подключений.  
**Participants Service** хранит состав участников каждой комнаты (звонка) в **Postgres** и кэширует эти данные в **Aerospike**.  
Если пользователь покидает звонок или подключается заново, информация мгновенно обновляется.

### Message Service
**Сервис сообщений** хранит чат звонка и статусы сообщений (“доставлено”, “прочитано”).  
Он использует **Aerospike** для быстрой записи и чтения, а также для горизонтального масштабирования при большом количестве параллельных звонков.

### Процесс доставки сообщения в чате звонка
1. **Client → WSH:** пользователь отправляет сообщение в чат звонка.  
2. **WSH** сохраняет сообщение через **Message Service**, получает `message_id`.  
3. **WSH → Kafka:** сообщение публикуется в топик, соответствующий комнате.  
4. **Participants Message Handler** читает Kafka, получает список участников из **Participants Service**.  
5. **Handler → WSH’ов участников:** сообщение рассылается всем подключённым пользователям комнаты.  
6. **WSH** обновляют статусы сообщений (доставлено, прочитано) и уведомляют **Message Service**.

### Локальный кэш устройства
Если пользователь теряет соединение, его клиент временно сохраняет сообщения в локальной БД.  
После восстановления соединения они автоматически отправляются через WSH.
